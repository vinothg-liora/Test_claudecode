...
window.LioraCashFlowInit = function (rootEl) {
    'use strict';

    // Helper: scoped DOM queries
    var _byId = function(id) { return rootEl.querySelector('#' + id); };
    ...
    function showSaveToast(message, isError) {
        let toast = _byId('liora-save-toast');
        if (!toast) {
    ...
    function renderPagination() {
        ...
        container.querySelectorAll('.page-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                renderTable();
                rootEl.querySelector('.table-wrapper').scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            });
        });
    }
    ...
    $('#nav-logo-home').addEventListener('click', () => {
        ...
        const dashTab = rootEl.querySelector('.nav-tab[data-tab="dashboard"]');
        if (dashTab) dashTab.classList.add('active');
        $$('.tab-content').forEach(tc => tc.classList.remove('active'));
        const target = _byId('tab-dashboard');
    ...
    const ftFileInput = _byId('ft-file-input');
    const ftUploadZone = _byId('ft-upload-zone');
    ...
    async function updateStorageStatus() {
        const dot = _byId('storage-status-dot');
        const text = _byId('storage-status-text');
        if (!dot || !text) return;
        try {
    ...
    async function renderFileHistory() {
        const container = _byId('ft-file-history');
        if (!container) return;
        const history = await getFileHistory();
        if (history.length === 0) {
    ...
    function renderRulesRecap() {
        ...
        const learnedContainer = _byId('ft-recap-learned-list');
        const learnedCountEl = _byId('ft-recap-learned-count');
        ...
        const builtinContainer = _byId('ft-recap-builtin-list');
    ...
    rootEl.querySelectorAll('.ft-rules-tab').forEach(tab => {
        tab.addEventListener('click', () => {
            rootEl.querySelectorAll('.ft-rules-tab').forEach(t => t.classList.remove('active'));
            tab.classList.add('active');
            rootEl.querySelectorAll('.ft-rules-recap-panel').forEach(p => {
                p.classList.remove('active');
                p.style.display = 'none';
            });
            const target = _byId('ft-rules-recap-' + tab.dataset.rulesTab);
            if (target) {
    ...
    function renderLearnedRules() {
        wireValidateAll();
        wireClearLearned();
        const container = _byId('ft-learned-rules');
        const clearBtn = _byId('ft-clear-learned');
        const validateAllBtn = _byId('ft-validate-all');
        const countEl = _byId('dq-count-rules');
        if (!container) return;
    ...
    function wireValidateAll() {
        ...
        const btn = _byId('ft-validate-all');
        if (!btn) return;
    ...
    function wireClearLearned() {
        ...
        const btn = _byId('ft-clear-learned');
        if (!btn) return;
    ...
    $$('.nav-tab').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            btn.classList.add('active');
            $$('.tab-content').forEach(tc => tc.classList.remove('active'));
            const target = _byId('tab-' + btn.dataset.tab);
            if (target) target.classList.add('active');
    ...
    function renderDqMonthButtons() {
        const container = _byId('dq-filter-months');
        if (!container) return;
        container.innerHTML = '';
    ...
    _byId('dq-select-all').addEventListener('click', () => {
        dqSelectedMonths = null;
        renderDqMonthButtons();
        renderDataQuality();
    });

    _byId('dq-select-none').addEventListener('click', () => {
    ...
    function updateSuggestButtons() {
        ...
        rootEl.querySelectorAll('.dq-btn-suggest-all').forEach(btn => {
            btn.disabled = !hasKey;
        });
    }
...
- "raison" est une phrase courte en français.`
    ...
    function wireDqCollapsible() {
        ...
        rootEl.querySelectorAll('.dq-collapsible').forEach(section => {
    ...
    function renderDqTable(tbodyId, rows, categories, bulkBtnId, suggestBtnId) {
        const tbody = _byId(tbodyId);
        const bulkBtn = _byId(bulkBtnId);
        const suggestBtn = _byId(suggestBtnId);
        if (rows.length === 0) {
    ...
    function renderSimProjection() {
        ...
        // Detail table
        const thead = _byId('sim-proj-thead');
        const tbody = _byId('sim-proj-tbody');
    ...
    function updateModelExplanation() {
        const el = _byId('sim-model-explain');
        if (el) el.innerHTML = MODEL_EXPLANATIONS[simActiveModel] || '';
    }

    // ── Model toggle ──
    rootEl.querySelectorAll('.sim-model-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            rootEl.querySelectorAll('.sim-model-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
    ...
    function buildSimInputs() {
        ...
        // Enc inputs
        const encContainer = _byId('sim-enc-inputs');
        ...
        // Dec inputs
        const decContainer = _byId('sim-dec-inputs');
    ...
    function runSimulation() {
        ...
        // Read enc inputs
        const encInputs = {};
        rootEl.querySelectorAll('#sim-enc-inputs input[data-field="amount"]').forEach(inp => {
            encInputs[inp.dataset.cat] = { amount: parseFloat(inp.value) || 0 };
        });
        rootEl.querySelectorAll('#sim-enc-inputs input[data-field="delay"]').forEach(inp => {
            if (encInputs[inp.dataset.cat]) encInputs[inp.dataset.cat].delay = parseInt(inp.value) || 0;
        });

        // Read dec inputs
        const decInputs = {};
        rootEl.querySelectorAll('#sim-dec-inputs input[data-field="decAmount"]').forEach(inp => {
        ...
        // Detail table
        const thead = _byId('sim-manual-thead');
        const tbody = _byId('sim-manual-tbody');
        thead.innerHTML = `<tr><th>Catégorie</th>${futureKeys.map(k => `<th class="text-right">${monthKeyToLabel(k)}</th>`).join('')}<th class="text-right">Total</th></tr>`;
    ...
    rootEl.addEventListener('mousemove', (e) => {
        rootEl.style.setProperty('--mouse-x', e.clientX + 'px');
        rootEl.style.setProperty('--mouse-y', e.clientY + 'px');
    });
};
